...
static bool	solve_equation(t_vec k, t_vec ray_origin, double *t)
{
	double	u;
	double	v;
...
static bool	compute_intersection(t_ray tfm_ray, t_poi *poi)
{
	...
	k = tfm_ray.ab;
	vec_normalize(&k);
	if (!solve_equation(k, tfm_ray.pa, &t))
		return (false);
...
bool	plane_intersect(t_ray cam_ray, t_poi *poi, t_obj_lst *cur_obj)
{
	...
	new_origin = gtfm_vec_apply(cur_obj->obj.gtfm, origin, FWD);
	poi->normal = vec_sub(gtfm_vec_apply(cur_obj->obj.gtfm, normal_vec, FWD),
						  new_origin);
